...
def grade(num):
    if num >= 90:
        ...
        return (
            "Your performance is below average... There is massive scope of improvement... You have barely passed the exam with grade E.")
    else:
...
def course_graph():
    ...
    with open(f'{path}/Course.csv', 'r') as f:
        ...
        for j in lst:
            ...
            leg = plt.legend(fontsize=10, loc="upper right", facecolor="Black", edgecolor="Black",
...
def batch_graph(arg):
    ...
    grade_no_lst = {'A': grade_lst.count('A'), 'B': grade_lst.count('B'), 'C': grade_lst.count('C'),
                    'D': grade_lst.count('D'), 'E': grade_lst.count('E'), 'F': grade_lst.count('F')}
    ...
    patches, texts = ax.pie(sizes, labels=new_labels, colors=color_lst, explode=explode, shadow=True, startangle=-90,
...
for i in range(student_no):
    ...
    if duplicate('Student.csv', student_id, 0):
        ...
        print(
            'please enter the subjects marks in the above mentioned order in a list type and if you dont have a particular subject write there "null" (e.g. [100,100,"null",75,69,85])')
